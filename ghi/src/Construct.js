...
function NewDummyForm() {
  ...
  const handleSubmit = async (event) => {
    ...
    const URL = `${process.env.REACT_APP_USER_SERVICE_API_HOST}/test`;
    const fetchConfig = {
      method: "POST",
          ...
          <form onSubmit={handleSubmit} id="manufacturer-form">
            ...
            <div className="form-floating mb-3">
              <input
                ...
                onChange={(event) =>
                  setRequiredUnlimitedText(event.target.value)
                }
                required
                placeholder="requiredUnlimitedText"
              />
              <label htmlFor="requiredUnlimitedText">
                requiredUnlimitedText
              </label>
            </div>
